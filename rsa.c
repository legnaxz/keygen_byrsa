...
long long
rsa_modExp (long long b, long long e, long long m)
{
  if (b < 0 || e < 0 || m <= 0)
    {
      exit (1);
...
void
_rsa_gen_keys (struct public_key_class *pub, struct private_key_class *priv)
{
...
// it is given. 
void
rsa_gen_keys (struct public_key_class *pub, struct private_key_class *priv,
...
unsigned char *
rsa_decrypt (const long long *message,
	     const unsigned long message_size,
